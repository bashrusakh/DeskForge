...
const PORT = 21116;
// Use server from config instead of hardcoded public servers
const apiServer = localStorage.getItem("api-server") || "";
const defaultHost = apiServer ? new URL(apiServer).hostname : location.hostname;
let HOST = localStorage.getItem("rendezvous-server") || localStorage.getItem("custom-rendezvous-server") || defaultHost;
//根据协议设置为ws或wss
...
// Server config is loaded from api-server in localStorage

function getDefaultUri(isRelay: Boolean = false): string {
  // Use custom-rendezvous-server, then rendezvous-server, then api-server host
